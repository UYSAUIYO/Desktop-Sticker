...
自研轻量测试框架（`dtest`），当前 **196 项**全部通过，覆盖配置存储、布局数学、图标分类、拼音检索、热键判定、时钟文案，以及动态壁纸的盘符选择、暂停优先级、档位解析、帧调度、ffmpeg 命令行构造、JSON 存储容错与媒体库源文件安全，与资源管理器的字节/百分比格式化、存储分类规则、模块归属判定、CPU 采样计算、JSON 响应组装。仅支持 Release（本机 Debug CRT 环境问题）。
