...
> **阶段 5（模型）已落地并实机验证（2026-09-25）**：内置 32 面体（截角二十面体，12 五边形 + 20 六边形）
> 已渲染进桌面壁纸，位于图标/分区卡片/时钟之下，60fps。几何不走文件加载，而是**程序化生成**：
> 60 个顶点 + 自写的通用凸包（参考实现用 Three.js 的 ConvexGeometry），因此可以单测
> （32 面 / 12+20 / V-E+F=2 / 共面 / 外向法线 / 116 个三角形）。相机是内置环绕（壁纸不吃输入，
> 故参考实现的"拖拽旋转/滚轮缩放"不适用），光照为单向光 + Blinn-Phong，配色沿用参考实现那 6 色。
> 场景选择：`shader/` 里有 `.frag` → 全屏着色器，否则 → 内置模型；`scene.json` 可显式指定。
> **仍未做**：`.obj/.gltf/.glb` 文件加载、粒子预设、用户 GLSL 的运行时编译。
