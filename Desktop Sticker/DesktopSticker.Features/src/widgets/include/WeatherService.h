...
// 后台线程立即拉取，失败 20 秒重试（6 次后放宽到 10 分钟），成功 30 分钟刷新；
// UI 线程通过 Snapshot() 读加锁快照。
...
private:
    ...
    static bool LocateIpApi(double& lat, double& lon, std::wstring& country, std::wstring& city);
    static bool LocateBaidu(double& lat, double& lon, std::wstring& country, std::wstring& city);
