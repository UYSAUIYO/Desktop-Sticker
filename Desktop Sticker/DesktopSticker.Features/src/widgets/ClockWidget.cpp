...
void ClockWidget::OnPaint() {
    ...
    // 公转角：匀速 24 小时一圈（每小时 15°），正午锚定在右半亮弧中点、午夜在左半暗弧中点。
    // 亮弧 = 白天（04:51 日出在右上 → 18:16 日落在左下，跨右半圈），暗弧 = 夜间（左半圈）；
    // 地球顺时针（φ 递增），16:4x 时位于下弧、日落后即进入左侧暗弧
    const float orbitU = ((static_cast<float>(nowMin_) - 720.0f) / 1440.0f) * 2.0f * kPi + kPi / 2.0f;
    ...
    // 左半圈：暗环段 = 夜间（细且暗；先画，经过太阳圆面的段落被太阳自然遮挡）
    ...
    for (int i = kSegs / 2; i < kSegs; ++i) {
        ...
        const float depth = 0.5f + 0.5f * cosf((u0 + u1) * 0.5f); // cos：右半圈(白天)亮、左半圈(夜间)暗
        ringBrush_->SetColor(D2D1::ColorF(0xFFFFFF, 0.12f + 0.25f * depth));
        orbitArc(u0, u1, 0.9f + 0.7f * depth, ringBrush_);
    }
    ...
    // 右半圈：亮环段 = 白天（粗且亮，压在太阳前）+ 地球（白天走在亮弧上）
    for (int i = 0; i < kSegs / 2; ++i) {
        const float u0 = i * 2.0f * kPi / kSegs;
        const float u1 = (i + 1) * 2.0f * kPi / kSegs;
        const float depth = 0.5f + 0.5f * cosf((u0 + u1) * 0.5f);
        ringBrush_->SetColor(D2D1::ColorF(0xFFFFFF, 0.25f + 0.60f * depth));
        orbitArc(u0, u1, 1.3f + 1.7f * depth, ringBrush_);
    }
    if (planetFront) drawPlanet();
